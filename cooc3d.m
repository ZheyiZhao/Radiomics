...
%Calculations
    ...
    for j=1:numLevels
        for i=1:numLevels
            ...
            tempTen = tempTen+ (((i + j - mux - muy)^2) .* (value));
            if i~=j
                tempInVar=tempInVar+ value/(i-j)^2;
            end
            tempProminence= tempTen+ (((i + j - mux - muy)^4) .* (value));
        end
    end
